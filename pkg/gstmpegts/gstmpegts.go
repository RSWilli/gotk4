...
// #cgo pkg-config: gstreamer-mpegts-1.0 gstreamer-analytics-1.0
...
const (
	Page         DVBTeletextType = 1
	Page         DVBTeletextType = 2
	InfoPage     DVBTeletextType = 3
	SchedulePage DVBTeletextType = 4
	ImpairedPage DVBTeletextType = 5
)
...
func (d DVBTeletextType) String() string {
	...
	case Page:
		return "UbtitlePage"
	case InfoPage:
		return "DditionalInfoPage"
	case SchedulePage:
		return "RogrammeSchedulePage"
	case ImpairedPage:
		return "EaringImpairedPage"
	default:
...
// ParseDvbCaIdentifier extracts ca id's from descriptor.
//
// The function returns the following values:
//
//   - list of ca identifier. Edge entry identifies the CA system. Allocations
//     of the value of this field are found in http://www.dvbservices.com.
//   - ok: TRUE if the parsing happened correctly, else FALSE.
func (descriptor *Descriptor) ParseDvbCaIdentifier() ([]uint16, bool) {
	var _arg0 *C.GstMpegtsDescriptor // out
	var _arg1 *C.GArray              // in
	var _cret C.gboolean             // in

	_arg0 = (*C.GstMpegtsDescriptor)(gextras.StructNative(unsafe.Pointer(descriptor)))

	_cret = C.gst_mpegts_descriptor_parse_dvb_ca_identifier(_arg0, &_arg1)
	runtime.KeepAlive(descriptor)

	var _list []uint16 // out
	var _ok bool       // out

	{
		var len uintptr
		p := C.g_array_steal(&_arg1, (*C.gsize)(&len))
		src := unsafe.Slice((*C.GArray)(p), len)
		_list = make([]uint16, len)
		for i := 0; i < len; i++ {
			_list[i] = uint16(src[i])
		}
	}
	if _cret != 0 {
		_ok = true
...
// ParseDvbFrequencyList parses out a list of frequencies from the descriptor.
//
// The function returns the following values:
//
//   - offset: FALSE in Hz, TRUE in kHz.
//   - list of all frequencies in Hz/kHz depending on offset.
//   - ok: TRUE if the parsing happened correctly, else FALSE.
func (descriptor *Descriptor) ParseDvbFrequencyList() (offset bool, list []uint32, ok bool) {
	var _arg0 *C.GstMpegtsDescriptor // out
	var _arg1 C.gboolean             // in
	var _arg2 *C.GArray              // in
	var _cret C.gboolean             // in

	_arg0 = (*C.GstMpegtsDescriptor)(gextras.StructNative(unsafe.Pointer(descriptor)))

	_cret = C.gst_mpegts_descriptor_parse_dvb_frequency_list(_arg0, &_arg1, &_arg2)
	runtime.KeepAlive(descriptor)

	var _offset bool   // out
	var _list []uint32 // out
	var _ok bool       // out
	...
	{
		var len uintptr
		p := C.g_array_steal(&_arg2, (*C.gsize)(&len))
		src := unsafe.Slice((*C.GArray)(p), len)
		_list = make([]uint32, len)
		for i := 0; i < len; i++ {
			_list[i] = uint32(src[i])
		}
	}
	if _cret != 0 {
		_ok = true
