...
func UnsafeRegisterSubClass[InstanceT Object, ClassT, OverridesT any](
	...
	parentInstanceFromUnsafePointer func(unsafe.Pointer) InstanceT,
	...
	data := &subClassData[InstanceT]{
		classInit: func(gclass unsafe.Pointer) {
			...
			gtype := unsafeTypeFromObject(gclass)

			// register the signals
			for name, signal := range signals {
				signal.withInstanceType(gtype).registerFor(name, gtype)
			}
...
func (sd SignalDefinition) withInstanceType(instanceType Type) SignalDefinition {
	// add the instance type to the param types
	sd.ParamTypes = append([]Type{instanceType}, sd.ParamTypes...)
	return sd
}
